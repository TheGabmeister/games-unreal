...
- IA assets generated by the DiabloEditor plugin under `Content/Input/Actions/` (`IA_Click` mapped to LMB, `IA_Cast` mapped to RMB, `IA_CharPanel` mapped to C, `IA_Inventory` mapped to I, `IA_Spellbook` mapped to S, `IA_Menu` mapped to ESC, `IA_Move`, `IA_Look`). IMC at `Content/Input/IMC_Diablo`.
