...
- **Splash damage:** rockets and grenades deal full damage inside a 60 u inner plateau, then fall off linearly to 0 at the 120 u outer radius (`DamageInnerRadius = 60`, `DamageOuterRadius = 120`, linear falloff). The inner plateau is an implementation workaround for a UE quirk in `ApplyRadialDamageWithFalloff` that measures distance to the victim's capsule center rather than its surface — see CLAUDE.md "Radial damage measures to the victim's component center" for the engine-level explanation. Design intent is "direct hit = full damage, splash falls off with distance"; the plateau is how the engine is coerced into delivering that.
...
- **Knockback:** applied as an instantaneous impulse along the hit normal (or away from the explosion center). Knockback magnitude scales with the damage dealt, so it inherits the same 60 u plateau + linear falloff shape as the splash damage curve. Splash applies to the player as well, enabling rocket jumps.
- **Grenades** bounce off surfaces (restitution 0.4) and explode under one of three conditions (first to occur wins):
  ...
  `Explode()` calls `UGameplayStatics::ApplyRadialDamageWithFalloff` with the same parameters as the Rocket (see the splash-damage bullet above: BaseDamage 100, inner plateau 60, outer radius 120, linear falloff, `UQuakeDamageType_Explosive`) and then `Destroy()`s. The Ogre's grenade (`AQuakeProjectile_OgreGrenade`) reuses the same base class with different stats (speed 600, damage 40, same splash curve).
