...
**Row names in `DT_EnemyStats` / `DT_WeaponStats` must match the `StatsRowName` in each leaf C++ constructor.** Current keys: `Grunt`, `Knight`, `Ogre` (enemies); `Axe`, `Shotgun`, `Nailgun`, `RocketLauncher` (weapons). Missing rows fall back to C++ defaults — tests never configure the tables and exercise the C++ fallback path only, which keeps them world-free.
...
- **`AQuakePlayerState`** — current-level stats (kills, secrets, deaths, `GetTimeElapsed()` computed from world time) and active powerups (`TArray<FQuakeActivePowerup>`). Auto-cleared on `OpenLevel` only — UE preserves PlayerState across pawn death/respawn, so cumulative stats persist across the level attempt and must NOT be reset by the death-restart path. Keys are planned to live here too (their lifecycle matches powerups, not weapons/ammo/armor) and will land with the key-pickup work in Phase 8; the `ClearPerLifeState()` method that empties powerups + keys on death-restart will land alongside the Phase 6.4 failure-loop implementation. Neither exists yet — adding them preemptively is out of scope, but SPEC section 1.4 is the authoritative reference for where they'll live. Don't assume "PlayerState resets on death" without that call; use `OpenLevel` for level transitions. **Tick is gated on powerups:** the PlayerState starts with tick disabled; granting a powerup must call `EnablePowerupTick()`, and Tick auto-disables itself when the last powerup expires. Elapsed time is computed on demand (no tick accumulation), so per-level stats don't waste a tick when no powerups are active.
- **`AQuakeCharacter`** — live health, `WeaponInstances[NumWeaponSlots]` (actor pointers for each owned slot), and `CurrentWeapon` / `CurrentWeaponSlot`. Tied to the body; destroyed on death and respawn. Exposes facade methods `GiveAmmo` / `ConsumeAmmo` / `GetAmmo` (forward to the GameInstance) and `GiveHealth(Amount, bOvercharge)` (self-owned state, caps at `MaxHealth` or `GetOverchargeCap()` = 200). `NumWeaponSlots` (= 8) is a `static constexpr` on `AQuakeCharacter` — use it everywhere the 8-slot count is needed (tests, `UQuakeGameInstance::OwnedWeaponClasses.SetNum`, auto-switch mask sizing) rather than hardcoding `8`.
...
**`Fire` preamble is centralized.** Every weapon subclass's `Fire` starts with `APawn* PawnInstigator; UWorld* World; if (!GetFireContext(InInstigator, PawnInstigator, World)) return;` — the base class helper handles the pawn cast + world null-check in one call. Do not re-implement the boilerplate in new weapons; use `GetFireContext`.
...
**Splash damage lives on the projectile subclass, not on the damage type.** SPEC section 1.5 explicitly calls this out: `UQuakeDamageType_Explosive` carries the self-damage scale (0.5) and knockback scale (4.0), but `BaseDamage`, `DamageInnerRadius`, and `SplashRadius` are `UPROPERTY` defaults on [AQuakeProjectile_Rocket](Source/Quake/QuakeProjectile_Rocket.h). The rocket is the thing that knows where it exploded. `HandleImpact` applies splash via `UGameplayStatics::ApplyRadialDamageWithFalloff` with `DamageInnerRadius = 60`, `DamageOuterRadius = 120`, `DamageFalloff = 1.0` (linear). Direct-hit victims fall inside the inner plateau and take full `BaseDamage`; do not call `ApplyPointDamage` separately (it would double-count the direct hit).

**Radial damage measures to the victim's component center, not its surface — this is why `DamageInnerRadius` is non-zero.** `UGameplayStatics::ApplyRadialDamageWithFalloff` internally calls `ComponentIsDamageableFrom`, which line-traces from `Origin` to `VictimComp->Bounds.Origin`. When `Origin` sits on or inside the victim's collision (i.e., the direct-hit case), the trace produces no blocking hit and the helper falls back to `OutHitResult.ImpactPoint = VictimComp->GetComponentLocation()` — the capsule center. `AActor::InternalTakeRadialDamage` then computes `Distance = |ImpactPoint - Origin|`, which for a direct rocket hit on a 35 u-radius / 90 u-half-height capsule works out to roughly `sqrt(35² + z_offset²) ≈ 37 u`, and with `InnerRadius = 0` scales `BaseDamage` by `(1 - 37/120) ≈ 0.69`. Plain-language consequence: **`InnerRadius = 0` with `ApplyRadialDamageWithFalloff` never delivers `BaseDamage` to a live pawn, even on a dead-center capsule hit.** We work around this by setting `DamageInnerRadius = 60` on the rocket so the ~37 u measurement lands inside the full-damage plateau. The alternative pattern — `ApplyPointDamage` on the direct-hit actor plus `ApplyRadialDamageWithFalloff` with that actor in `IgnoreActors` — is more faithful to "inner radius 0, linear from center" semantics but costs a second call; revisit if we need bone-accurate damage against skeletal-mesh bosses. `AQuakeProjectile_Grenade::HandleImpact` has the same quirk and still uses `InnerRadius = 0` — it should adopt the same plateau when we next touch it.

**The linear falloff formula is extracted as a pure static helper** `AQuakeProjectile_Rocket::ComputeLinearFalloffDamage(BaseDamage, Distance, Radius)` so it can be unit-tested without spinning up a world — same pattern as `ApplyArmorAbsorption`, `ApplyQuakeAirAccel`, and `ComputePainChance`. The helper describes the *shape* of the falloff curve outside the inner plateau (full at `d=0`, zero at `d>=r`, linear between) but deliberately does not model `DamageInnerRadius` — it was written against the SPEC's idealized falloff, not the engine's actual call path. Keep that in mind when reading the `Quake.Weapon.RocketSplash.*` test suite: the tests regress the formula, not the in-game damage curve.
...
**Grenades (Phase 7) have a three-way detonation rule** (SPEC 2.2). [AQuakeProjectile_Grenade::HandleImpact](Source/Quake/QuakeProjectile_Grenade.cpp) explodes on `ACharacter` contact and returns (no-op) on world geometry hits — letting `UProjectileMovementComponent::bShouldBounce = true, Bounciness = 0.4` handle the bounce. The fuse is a 2.5 s `FTimerHandle` set in `BeginPlay`, never reset by bouncing. The firer is ignored for a 0.25 s grace period after spawn (`FirerGraceEndTime` compared against `GetTimeSeconds`) — after that, walking into your own grenade detonates it. `OnProjectileBounce` is wired via `AddDynamic` in the ctor for the bounce-sound stub (Phase 14 wires real audio). Splash damage reuses `UQuakeDamageType_Explosive` and the same `ApplyRadialDamageWithFalloff(InnerRadius 0, DamageFalloff 1.0)` call as the rocket; only the projectile class and flight parameters differ.
...
- **[AQuakeEnemy_Grunt](Source/Quake/QuakeEnemy_Grunt.h)** / **AQuakeAIController_Grunt** is the Phase 3 reference pair. The Grunt overrides `FireAtTarget` for the hitscan rifle (trace on the `Weapon` channel with `UQuakeDamageType_Bullet`). `AQuakeAIController_Grunt` is an empty marker subclass — the base controller's FSM is sufficient. Follow this split for every new enemy type: the leaf pawn sets stats + overrides its attack, the leaf controller only exists when there is per-type FSM behavior to add.
- **[AQuakeEnemy_Knight](Source/Quake/QuakeEnemy_Knight.h)** / **AQuakeAIController_Knight** (Phase 7) is another marker-subclass pair — melee trace in `FireAtTarget`, base FSM charges directly.
- **[AQuakeEnemy_Ogre](Source/Quake/QuakeEnemy_Ogre.h)** / **AQuakeAIController_Ogre** (Phase 7) is the per-type FSM template. The Ogre has two attacks (grenade lob at range, chainsaw melee in close), so `FireAtTarget` is the melee trace and a separate `FireGrenadeAtTarget` spawns the grenade; the controller's `Tick` override intercepts the `Attack` state and picks one based on `DistToTarget <= MeleeThreshold`. `AttackRange` on the pawn is set to the grenade range (1500 u) so the base Chase→Attack transition fires early enough to reach melee range.

**Per-state Tick override gotcha.** When a controller subclass overrides `Tick` to intercept a single FSM state, it must NOT call `Super::Tick` for that state — the base's `switch` would also run its version (e.g., base Attack calls `Enemy->FireAtTarget`, causing a double-fire on every tick). Instead, call `AAIController::Tick` (the grandparent) and manually replicate the pre-switch bookkeeping the base does (`TimeInState += DeltaTime`, Dead/null-pawn check, grudge expiry). [AQuakeAIController_Ogre::Tick](Source/Quake/QuakeAIController_Ogre.cpp) is the canonical pattern; copy it for any future per-state override.

**Infighting (Phase 7, SPEC 3.3).** When an enemy's `TakeDamage` runs with an `EventInstigator` whose pawn is another `AQuakeEnemyBase` (not the player, not a hazard), the controller's `OnDamaged` seeds a grudge: `GrudgeTarget = InstigatorPawn`, `GrudgeExpireTime = Now + GrudgeDuration` (default 10 s). `Tick` clears the grudge and reverts to `PlayerController->GetPawn()` when the timer expires or the grudge target dies. `OnTargetPerceptionUpdated` accepts stimuli from the player OR the active grudge target so LKL stays fresh during infighting. No infighting-specific damage code is required — the emergent behavior falls out of `EventInstigator`-driven target switching alone.
...
**Drop table** is a `TArray<FQuakeDropEntry> DropTable` UPROPERTY on `AQuakeEnemyBase`. Each entry is `{ TSubclassOf<AQuakePickupBase> PickupClass, int32 Quantity, float Chance }`. Filled per-enemy-type in the BP subclass (e.g., `BP_Enemy_Grunt` → `[{ BP_Pickup_BackpackShells, 5, 1.0 }]` per SPEC 3.2). `AQuakeEnemyBase::SpawnDrops` iterates the table at death, rolls `FMath::FRand()` per entry against `Chance`, and spawns pickups at the enemy's location. Drops spawn only on non-gib deaths.

**`TSubclassOf<AQuakePickupBase>` in `FQuakeDropEntry` requires the full `AQuakePickupBase` header in `QuakeEnemyBase.h`, not a forward decl** — same gotcha as `TSubclassOf<AQuakeWeaponBase>` in `QuakeGameInstance.h`. The include must come *before* `QuakeEnemyBase.generated.h` (UHT rejects includes after the generated header).

**Gib detection** happens at the end of `TakeDamage` before calling `Die`. The rule (SPEC 3.4): overkill is `ScaledDamage - HealthBefore`; gib when `Overkill >= HealthBefore * 2` (i.e., damage is at least 2× remaining HP). `HealthBefore` is reconstructed from `Health + ScaledDamage` since `Health` is already decremented above. `Die(Killer, bGibbed)` dispatches to `PlayGibReaction` (Phase 7 stub: hide the actor) vs `PlayDeathReaction` (collapse), and skips `SpawnDrops` when `bGibbed` is true. The `AQuakeCharacter` (player) path uses a different death flow and is NOT gib-aware — the overkill rule is enemy-only.

## Architecture: Activation Chains

Buttons, triggers, doors, spawn points, and the level exit communicate via the `IQuakeActivatable` interface — a **pure C++ virtual** `Activate(AActor* InInstigator)`, **not** a `BlueprintNativeEvent`. Do not write `Activate_Implementation` (that suffix is only valid for `BlueprintNativeEvent` methods, and using it with this interface will not compile). Sources hold typed `TArray<TObjectPtr<AActor>>` slots filled per-instance in the editor via the actor picker (eyedropper UX). **No string-name targeting** — Quake's original `targetname`/`target` lookup is intentionally not used because it loses refactor traceability and editor pick-list filtering. See SPEC sections 5.5 and 5.6.

**Always name the interface parameter `InInstigator`, not `Instigator`.** `AActor` already has a protected `TObjectPtr<APawn> Instigator` member, so every override that shadowed the parameter name tripped `error C4458: declaration hides class member` under UE's warnings-as-errors build. The interface signature uses `InInstigator` for this reason, and every `AQuakeTrigger_*`, `AQuakeDoor`, and `AQuakeButton::Fire` follows the same convention.

**Trigger hierarchy.** [AQuakeTrigger](Source/Quake/QuakeTrigger.h) is the `UCLASS(Abstract)` base. It owns a `UBoxComponent TriggerVolume` (query-only, overlap Pawn) and a `TArray<TObjectPtr<AActor>> Targets`. Its `Activate(InInstigator)` default impl calls `FireTargets(InInstigator)`, iterating the array and calling `IQuakeActivatable::Activate` on each valid entry (null entries and non-IQuakeActivatable entries log a warning but don't crash — authoring errors are loud). The base class also binds `OnComponentBeginOverlap` in `BeginPlay`, so a placed trigger fires on pawn entry automatically. Subclasses override `Activate` to add per-type behavior before/after the `Super::Activate` fan-out:

- **`AQuakeTrigger_Relay`** — no-op override; fan-out only. For chaining or fan-out without pawn entry.
- **`AQuakeTrigger_Hurt`** — **self-contained, does NOT call Super::Activate.** Starts a per-pawn repeating damage timer on overlap (same pattern as `AQuakeHazardVolume`, see below) firing `DamagePerTick` via `UQuakeDamageType_Telefrag`. Clears the timer in `EndOverlap`. Kills volumes and crusher pits.
- **`AQuakeTrigger_Teleport`** — teleports a `ACharacter` instigator to `Destination`'s transform. Velocity magnitude is preserved and rotated to the destination's yaw so momentum carries through the portal. Non-character instigators (e.g., a relay firing the teleport) still propagate `Super::Activate` for the chain.
- **`AQuakeTrigger_Message`** — calls `AQuakeHUD::ShowMessage(FText, float)` then `Super::Activate`. The HUD's transient message system is a `DrawHUD` Canvas draw with an expiry time (see `AQuakeHUD::ShowMessage`).
- **`AQuakeTrigger_Secret`** — one-shot (guarded by `bCredited` so re-entry doesn't re-credit). Logs + HUD message; the actual `PlayerState->SecretsFound++` wiring lands in Phase 9 when the stat lives.
- **`AQuakeTrigger_Exit`** — `UGameplayStatics::OpenLevel(NextMapName)`. Fires `Super::Activate` FIRST so on-exit bookkeeping triggers run before the world swap.
- **`AQuakeTrigger_Spawn`** — Phase 9 stub. Class exists so buttons can reference it in the editor; no typed `SpawnPoints` field yet because `AQuakeEnemySpawnPoint` lands in Phase 9.

**Buttons are input sources, not chainables.** [AQuakeButton](Source/Quake/QuakeButton.h) **does not implement `IQuakeActivatable`**. It holds its own `Targets` list and fires it on Touch-overlap or Shoot-hit. For a "fire on signal" relay, use `AQuakeTrigger_Relay`. The collision is set up to support both touch and shoot simultaneously (overlap Pawn, block Weapon + Visibility) — `ActivationMode` gates which signal actually calls `Fire`, so authoring a touch-only button that also stops bullets is just setting the mode.

## Architecture: Doors

[AQuakeDoor](Source/Quake/QuakeDoor.h) is **tick-driven linear interpolation, not `UTimelineComponent`.** Timeline curves require a `.uasset` authored in the editor, which would pull gameplay data out of C++ and violate the "C++ first" rule. A linear ramp between two relative locations is deterministic, has no asset dependency, and matches original Quake's door feel. The state machine is `Closed → Opening → Open → Closing → Closed` in [AQuakeDoor::Tick](Source/Quake/QuakeDoor.cpp); `Opening` and `Closing` both step the relative location by `speed * DeltaSeconds` toward the target. Do NOT resurrect `UTimelineComponent` for doors without a strong reason — the curve-asset dependency is the whole thing the project is trying to avoid.

**Closing safety is two-layer.** (1) Before transitioning `Open → Closing`, `TryStartClosing()` queries `BlockingZone->GetOverlappingActors(..., ACharacter::StaticClass())`; if anyone's in the doorway it re-arms the auto-close timer for 0.5 s and retries. This prevents a slow-closing door from starting to close around a walking pawn. (2) Once already `Closing`, the mesh's `SetRelativeLocation(..., bSweep=true)` fires `OnComponentHit` — `HandleCrushHit` applies 10000 damage with `UQuakeDamageType_Telefrag` to any `ACharacter` the mesh pushes into, and the move completes (stopping would trap the corpse). The two cases are different moments — pre-close = gate, mid-close = crush — and both are needed.

**Keys are a Phase 10 stub.** `EQuakeKeyColor RequiredKey` exists on doors, but `CanOpenFor(AActor*)` always returns false for any non-None key — the PlayerState key query doesn't exist yet. Locked doors are therefore permanently locked until Phase 10 lands. The locked-feedback path (`GEngine->AddOnScreenDebugMessage`) is a stub; Phase 10 replaces it with a proper `AQuakeHUD::ShowMessage` "You need the [color] key" line.

## Architecture: Hazard Volumes

[AQuakeHazardVolume](Source/Quake/QuakeHazardVolume.h) handles lava and slime via a **per-pawn `FTimerHandle` map** keyed by weak pointer. On `BeginOverlap`: launch the character away from the volume center horizontally (SPEC 5.2 entry knockback — 200 u for lava, 0 for slime) and register a repeating timer at `TickRate` that applies `DamagePerTick` via `UGameplayStatics::ApplyPointDamage` with the configured damage type. On `EndOverlap`: clear that pawn's timer. `EndPlay` sweeps any remaining timers to avoid dangling callbacks on volume destruction.

**First damage tick fires at `TickRate`, not instantaneously on entry.** This matches original Quake's "damage over time" hazard semantics — the entry knockback is the on-contact punishment; the damage is the sustained punishment. If you want on-entry damage (e.g., for a crusher pit), call `ApplyTickDamage` directly from `BeginOverlap` — `AQuakeTrigger_Hurt` uses this "fire immediately + schedule repeat" pattern explicitly because kill volumes are supposed to bite the moment you step in.

**`UQuakeDamageType_Lava` carries `bSuppressesPain = true`** — `AQuakeEnemyBase::TakeDamage` and `AQuakeCharacter::TakeDamage` read this through the shared-base CDO cast, so burning in lava doesn't trigger a flinch animation every tick. `bCausedByWorld = true` attributes the kill to `<world>` in the death log and prevents the infighting grudge system from seeding an enemy-vs-enemy grudge when lava kills an enemy.

## Architecture: HUD Transient Messages

[AQuakeHUD::ShowMessage(FText, float Duration)](Source/Quake/QuakeHUD.h) stores a single active message with a world-time expiry; `DrawHUD` paints it centered near the top of the screen while active. Later calls replace any currently-visible message (no queue — the design intent is "latest message wins," matching original Quake's single-line message area). The width calculation uses `Canvas->ClipX`, which requires `#include "Engine/Canvas.h"` — `Canvas` is a `TObjectPtr<UCanvas>` forward-declared in `AHUD.h`, so accessing `->ClipX` in a cpp without the full header fails with `error C2027: use of undefined type 'UCanvas'`. Keep the include.
...
**Weapon-switch bindings use Enhanced Input's variadic `BindAction`** so a single `OnWeaponSlotPressed(const FInputActionValue&, int32 SlotIndex)` handler covers all slots — the slot index is passed as a bound extra arg at bind time. See the `WeaponBindings` table in [AQuakeCharacter::SetupPlayerInputComponent](Source/Quake/QuakeCharacter.cpp). Adding a new weapon slot = add the IA slot on the PlayerController, add the `{ Action, SlotIndex }` pair to the local `WeaponBindings` table. Don't add a new `OnWeaponNPressed` method per slot.
...
- **Enemy placements** — always `AQuakeEnemySpawnPoint` actors with `EnemyClass` set to a `BP_Enemy_*` class (`BP_Enemy_Grunt`, `BP_Enemy_Knight`, `BP_Enemy_Ogre`, …). Direct `BP_Enemy_*` placements in a level are decoration and **do not count** toward `KillsTotal` or gate the level-clear scan. This is the canonical authoring path for counted enemies; see SPEC section 5.1. `BP_Enemy_Ogre` must have `GrenadeClass` set to `BP_Projectile_Grenade` in its defaults or grenade lobs will no-op.
- **World-element BPs** (Phase 8) under `Content/Blueprints/World/` — thin subclasses of the Phase 8 C++ actors. Each only fills mesh / material / collision-extent slots; zero Event Graph nodes.
  - `BP_Door` (parent `AQuakeDoor`) — fills `DoorMesh` static mesh + material, sizes `BlockingZone` to the swept path. `RequiredKey` is set on the **placed instance**, not the BP, so a shared door BP can be reused across silver/gold/unlocked variants.
  - `BP_Button` (parent `AQuakeButton`) — fills `Mesh` + sizes `Collider`. `ActivationMode` (Touch/Shoot) is usually set on the BP; `Targets` is always per-instance.
  - `BP_HazardVolume_Lava` (parent `AQuakeHazardVolume`) — all C++ defaults already match lava (`UQuakeDamageType_Lava`, 30 dmg / 1 s, 200 u entry knockback). The BP just exists as a placement handle; `VolumeBox` extent is set per-instance. The visible lava surface is a separate `StaticMeshActor` with `MI_Lava` placed adjacent to the volume — the volume itself is invisible by design.
  - `BP_Trigger_Relay` / `_Hurt` / `_Teleport` / `_Message` / `_Secret` / `_Exit` (parents `AQuakeTrigger_*`) — almost no BP-level setup; `TriggerVolume` extent and the subclass-specific fields (`Destination`, `Message`, `NextMapName`, etc.) are all per-instance. `_Hurt` is self-contained — don't bother setting `Targets` on it, they're ignored.
  - `BP_Trigger_Spawn` is a Phase 9 stub; the class exists so buttons can reference it, but its typed `SpawnPoints` array lands with `AQuakeEnemySpawnPoint` in Phase 9.
- **DataTables** under `Content/Data/` — `DT_EnemyStats` (rows use `FQuakeEnemyStatsRow`) and `DT_WeaponStats` (rows use `FQuakeWeaponStatsRow`), both assigned in **Project Settings > Game > Quake** (see the Balance DataTables section above). Row names must match the `StatsRowName` FName set in each C++ constructor.
