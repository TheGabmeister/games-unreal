...
Unreal Engine 5.7 single-player FPS recreating original Quake gameplay with primitive shapes. The full design is in [SPEC.md](SPEC.md) — read it before making non-trivial gameplay changes. Module name: `Quake`. Current scope: the v1 milestone in `SPEC.md` section 11.

**SPEC.md is the design source of truth.** When SPEC and this file disagree on gameplay or data ownership, SPEC wins — update CLAUDE.md to match. CLAUDE.md is working notes for build/tooling/conventions, not a design doc.

**Read SPEC.md surgically.** ~1900 lines; reading top-to-bottom burns tens of thousands of tokens. Start with the `## Quick Map — Section Index` block in the first ~90 lines, then `Grep "^### <N>\.<M>"` or `Grep "^#### Phase <N>:"` for exact offsets, and `Read` with tight `limit=30-50` windows. Cross-references use SPEC section numbers (e.g., "SPEC 3.3") — those are stable; line numbers drift.

**Implementation phases.** v1 is built in 16 phases (Phase 0–15) defined in [SPEC.md section 11.5](SPEC.md). Each has explicit exit criteria. Find the current phase and stay within its scope. **Phase 1 (the strafe-jumping CMC) is the highest-risk item** — do not build combat code on top of an unverified CMC.
...
**Difficulty filter is partially wired.** [QuakeDifficulty.h](Source/Quake/QuakeDifficulty.h) defines `EQuakeDifficulty`; `AQuakeGameMode::CurrentDifficulty` defaults to Normal so spawn points filter via `CurrentDifficulty >= MinDifficulty`. Phase 12 moves storage to `UQuakeGameInstance` so it survives `OpenLevel`.
...
**Exit gating + level-end stats screen.** `AQuakeTrigger_Exit` honors `bGatedByClearCondition` (default true). When gated and unclear, shows a HUD message and bails. When clear: fires the base Targets chain (Phase 11 hooks the auto-save here), calls `AQuakeHUD::ShowLevelEndStats(StatsDisplaySeconds)`, schedules `OpenLevel(NextMapName)` via timer at the same duration. The Slate widget polls PlayerState/GameMode every paint — no explicit refresh needed. `bTransitionInFlight` blocks a second overlap from re-firing.
...
- **BP framework subclasses** under `Content/Blueprints/Framework/` — thin BPs holding asset/slot defaults C++ can't reference directly:
  - `BP_QuakeGameMode`, `BP_QuakeGameInstance` (owns `OwnedWeaponClasses[8]` slots — empty all-slots = weaponless spawn + warning), `BP_QuakePlayerController` (owns IA/IMC slots), `BP_QuakeCharacter` (mesh slots; watch the BP-shadows-C++ gotcha).
