...
private:
	...
	struct FSSEConnection
	{
		...
		bool bDirectRest = false;  // true for /api/call — returns plain JSON instead of SSE frame
	};
	...
	// Direct REST endpoint handler (/api/call — no MCP session required)
	void HandleDirectCall(FSocket* ClientSocket, const FString& Body);
