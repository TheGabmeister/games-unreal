...
void FMcpNativeTransport::HandleConnection(FSocket* ClientSocket)
{
	...
	// ── Direct REST endpoint: POST /api/call ──
	if (HttpReq.Path == TEXT("/api/call") && HttpReq.Method == TEXT("POST"))
	{
		HandleDirectCall(ClientSocket, HttpReq.Body);
		return;
	}
	...
	const TMap<FString, FString>& ExtraHeaders)
{
	...
	case 422: StatusText = TEXT("Unprocessable Entity"); break;
...
// ─── Direct REST Endpoint (/api/call) ───────────────────────────────────────

void FMcpNativeTransport::HandleDirectCall(FSocket* ClientSocket, const FString& Body)
{
	ISocketSubsystem* SocketSub = ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM);

	// Parse body: {"tool": "...", "arguments": {...}}
	...
	if (!FJsonSerializer::Deserialize(Reader, BodyObj) || !BodyObj.IsValid())
	{
		SendHttpResponse(ClientSocket, 400, TEXT("application/json"),
			TEXT("{\"success\":false,\"message\":\"Invalid JSON body\"}"));
		ClientSocket->Close();
	...
	FString ToolName;
	if (!BodyObj->TryGetStringField(TEXT("tool"), ToolName))
	{
		SendHttpResponse(ClientSocket, 400, TEXT("application/json"),
			TEXT("{\"success\":false,\"message\":\"Missing 'tool' field\"}"));
		ClientSocket->Close();
		if (SocketSub) SocketSub->DestroySocket(ClientSocket);
		return;
	}
	...
	const TSharedPtr<FJsonValue> ArgsValue = BodyObj->TryGetField(TEXT("arguments"));
	...
	{
		FString Msg = FString::Printf(TEXT("{\"success\":false,\"message\":\"Tool '%s' is not enabled\"}"), *ToolName);
		SendHttpResponse(ClientSocket, 404, TEXT("application/json"), Msg);
		ClientSocket->Close();
		if (SocketSub) SocketSub->DestroySocket(ClientSocket);
		return;
	}

	// Resolve dispatch action (same logic as HandleToolsCall)
	...
	if (ToolDef && !ToolDef->UsesToolNameDispatch())
	{
		...
		else
		{
			FString Msg = FString::Printf(
				TEXT("{\"success\":false,\"message\":\"Missing required '%s' field in arguments\"}"),
				*ActionField);
			SendHttpResponse(ClientSocket, 400, TEXT("application/json"), Msg);
			ClientSocket->Close();
			if (SocketSub) SocketSub->DestroySocket(ClientSocket);
			return;
		}
	...
	// Park socket — CompletePendingRequest writes the response when the handler finishes
	const FString RequestId = FGuid::NewGuid().ToString();
	{
		...
		Conn->bDirectRest = true;
		SSEConnections.Add(RequestId, Conn);
	}

	UE_LOG(LogMcpNativeTransport, Log,
		TEXT("/api/call: %s action=%s (RequestId=%s)"),
		*ToolName, *DispatchAction, *RequestId);

	// Dispatch on GameThread — response written by CompletePendingRequest
	...
	const TSharedPtr<FJsonObject>& Result, const FString& ErrorCode)
{
	...
	// Build response body — plain JSON for REST, JSON-RPC for SSE
	...
	if (Conn->bDirectRest)
	{
		...
		if (Result.IsValid()) { Resp->SetObjectField(TEXT("data"), Result); }
		if (!ErrorCode.IsEmpty()) { Resp->SetStringField(TEXT("errorCode"), ErrorCode); }
		FString JsonStr;
	...
	else
	{
		...
		ResponseBody = FMcpJsonRpc::BuildResponse(Conn->JsonRpcId, ToolResult);
	}
	...
	bool bIsDirectRest = Conn->bDirectRest;
	PendingAsyncWrites.fetch_add(1);

	Async(EAsyncExecution::ThreadPool,
		[this, Conn, ResponseBody = MoveTemp(ResponseBody),
		 CapturedRequestId, CapturedToolName, bCapturedSuccess, bIsDirectRest]()
	{
		{
			FScopeLock WriteLock(&Conn->WriteMutex);
			if (!Conn->Socket)
			{
				...
				return;  // Already cleaned up by Shutdown or client disconnected
			}

			if (bIsDirectRest)
			{
				SendHttpResponse(Conn->Socket,
					bCapturedSuccess ? 200 : 422,
					TEXT("application/json"), ResponseBody);
			}
			else
			{
				FString Frame = FString::Printf(
...
void FMcpNativeTransport::CleanupStaleRequests()
{
	...
	TArray<FString> Expired;
	{
		...
		for (const auto& [RequestId, Conn] : SSEConnections)
		{
			...
			// Never timeout direct REST calls — no socket, result stored in memory
			if (Conn->bDirectRest) continue;
			if (Now - Conn->StartTime > RequestTimeoutSeconds
