...
### Option A: Direct REST API (simplest — no MCP client needed)

The plugin exposes a direct REST endpoint at `/api/call` that requires no session management or protocol negotiation. Any tool that can make HTTP requests (curl, scripts, AI agents) can call it directly.

1. Enable in **Edit → Project Settings → Plugins → MCP Automation Bridge**:
   - Check **Enable Native MCP**
   - Set port (default: `3000`)
   - Check **Load All Tools on Start**
2. Restart the editor
3. Call tools directly:

```bash
curl -s http://localhost:3000/api/call \
  -H "Content-Type: application/json" \
  -d '{"tool":"manage_blueprint","arguments":{"action":"set_default","requestedPath":"/Game/MyBP","propertyName":"Mesh.SkeletalMeshAsset","value":"/Game/Meshes/SK_MyMesh.SK_MyMesh"}}'
```

**Request:** `{"tool": "<name>", "arguments": {...}}`
**Response:** `{"success": true, "message": "...", "data": {...}}`

No session init, no JSON-RPC envelope, no SSE streaming. One HTTP call per operation.

### Option B: Native MCP Transport (for MCP-compatible AI clients)
...
### Option C: TypeScript Bridge (classic setup)
