"""
Generate a rigged Spear Thrower enemy skeletal mesh for UE5.

Creates SK_SpearThrower.fbx — capsule torso with arms, thin cylinder spear on
right hand, rigged with armature (root, spine, head, arm_l, arm_r, spear, leg_l, leg_r).

Usage:
    blender --background --python generate_spearthrower.py -- [output_dir]

Output:
    SK_SpearThrower.fbx       — skeletal mesh (mesh + skeleton, bind pose)
    A_SpearThrower_Idle.fbx   — holds spear at ready
    A_SpearThrower_Windup.fbx — arm draws back
    A_SpearThrower_Throw.fbx  — arm snaps forward, releases spear
    A_SpearThrower_Death.fbx  — drops spear, falls backward
"""
...
# ---------------------------------------------------------------------------
# Spear Thrower Mesh — capsule torso, head, arms, legs, spear
# Faces -Y in Blender (rotated to +X for UE before export)
# ~0.70 Blender units tall (feet to head)
# ---------------------------------------------------------------------------

def build_spearthrower_mesh():
    single_mat = bpy.data.materials.new("M_SpearThrower_Placeholder")
    single_mat.use_nodes = True
    bsdf = single_mat.node_tree.nodes["Principled BSDF"]
    bsdf.inputs["Base Color"].default_value = (0.50, 0.35, 0.22, 1.0)
    ...
    # Right arm (raised, holding spear at ready)
    arm_r_upper = cylinder((0.15, 0, 0.38), 0.04, 0.14,
                           rot=(0, 0, math.radians(-15)))
    P.append(arm_r_upper)
    arm_r_lower = cylinder((0.17, -0.03, 0.47), 0.035, 0.12,
                           rot=(math.radians(-45), 0, math.radians(-10)))
    P.append(arm_r_lower)
    hand_r = sphere((0.17, -0.07, 0.53), 0.04)
    P.append(hand_r)

    # Spear — thin cylinder with pointed tip, held in right hand
    spear_shaft = cylinder((0.17, -0.07, 0.53), 0.012, 0.45,
                           rot=(math.radians(-70), 0, 0))
    P.append(spear_shaft)
    # Spear tip — small cone at the front end of the shaft
    spear_tip = cone((0.17, -0.27, 0.60), 0.025, 0.003, 0.06,
                     rot=(math.radians(-70), 0, 0), segs=8)
    P.append(spear_tip)
    ...
    # Headband (torus-like ring around head)
    for i in range(12):
        angle = i * math.pi * 2 / 12
        x = math.cos(angle) * 0.092
        z = 0.58 + math.sin(angle) * 0.005
        band = sphere((x, -0.02, z), 0.012)
        P.append(band)

    result = join(P)
    result.name = "SpearThrower_Mesh"
...
BONES = [
    ...
    ("arm_r",   (0.12, 0, 0.40),        (0.17, -0.07, 0.53),   "spine"),
    ("spear",   (0.17, -0.07, 0.53),    (0.17, -0.27, 0.63),   "arm_r"),
...
def build_armature():
    ...
    arm_obj.name = "SpearThrower_Armature"
    arm = arm_obj.data
    arm.name = "SpearThrower_Skeleton"
...
def make_idle(arm):
    """Holds spear at ready — 60 frames."""
    keys = []

    # Right arm holds spear raised, subtle readiness motion
    keys += [
        ("arm_r", 1,  (0.1, 0, -0.05)),
        ("arm_r", 30, (0.15, 0, 0.05)),
        ("arm_r", 60, (0.1, 0, -0.05)),
        ("spear", 1,  (0.03, 0, 0)),
        ("spear", 30, (-0.03, 0, 0)),
        ("spear", 60, (0.03, 0, 0)),
    ...
    return create_action(arm, "A_SpearThrower_Idle", 60, keys)


def make_windup(arm):
    """Arm draws back to throw — 15 frames."""
    keys = []
    ...
    # Frame 5: beginning to draw back — weight shifts to back foot
    keys += [
        ("arm_r", 5, (-0.4, 0, -0.15)),
        ("spear", 5, (-0.2, 0, 0)),
        ("spine", 5, (0.15, 0, 0.08)),
    ...
    # Frame 10: arm fully drawn back, body coiled
    keys += [
        ("arm_r", 10, (-0.8, 0, -0.25)),
        ("spear", 10, (-0.4, 0, 0.1)),
        ("spine", 10, (0.3, 0, 0.15)),
    ...
    # Frame 15: maximum coil — arm way back, about to release
    keys += [
        ("arm_r", 15, (-1.0, 0, -0.3)),
        ("spear", 15, (-0.5, 0, 0.15)),
        ("spine", 15, (0.35, 0, 0.2)),
        ("head",  15, (-0.15, 0, -0.08)),
        ("arm_l", 15, (0.35, 0, 0.15)),
        ("leg_l", 15, (0.25, 0, 0)),
        ("leg_r", 15, (-0.3, 0, 0)),
    ]

    return create_action(arm, "A_SpearThrower_Windup", 15, keys)


def make_throw(arm):
    """Arm snaps forward, releases spear — 15 frames."""
    keys = []

    # Frame 1: maximum coil (matches windup end)
    keys += [
        ("arm_r", 1, (-1.0, 0, -0.3)),
        ("spear", 1, (-0.5, 0, 0.15)),
        ("spine", 1, (0.35, 0, 0.2)),
    ...
    # Frame 4: explosive forward snap — arm whips forward
    keys += [
        ("arm_r", 4, (0.6, 0, 0.1)),
        ("spear", 4, (0.3, 0, -0.1)),
        ("spine", 4, (-0.2, 0, -0.1)),
    ...
    # Frame 7: full extension — spear released point
    keys += [
        ("arm_r", 7, (0.8, 0, 0.15)),
        ("spear", 7, (0.5, 0, -0.15)),
        ("spine", 7, (-0.25, 0, -0.12)),
    ...
    # Frame 11: follow-through, arm decelerating
    keys += [
        ("arm_r", 11, (0.5, 0, 0.05)),
        ("spear", 11, (0.3, 0, -0.05)),
        ("spine", 11, (-0.1, 0, -0.05)),
    ...
    return create_action(arm, "A_SpearThrower_Throw", 15, keys)


def make_death(arm):
    """Drops spear, falls backward — 30 frames."""
    keys = []

    # Frame 1: neutral
    for b in ["spine", "head", "arm_l", "arm_r", "spear", "leg_l", "leg_r"]:
        keys.append((b, 1, (0, 0, 0)))

    # Frame 6: drops spear, starts staggering
    keys += [
        ("spear", 6, (0.8, 0, 0.3)),
        ("arm_r", 6, (0.3, 0, 0.2)),
        ("arm_l", 6, (-0.1, 0, -0.2)),
    ...
    return create_action(arm, "A_SpearThrower_Death", 30, keys)
...
def main():
    ...
    print("  Building spear thrower mesh...")
    mesh_obj = build_spearthrower_mesh()
    ...
    sk_path = os.path.join(OUTPUT_DIR, "SK_SpearThrower.fbx")
