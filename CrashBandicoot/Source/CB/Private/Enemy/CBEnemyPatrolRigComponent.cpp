...
#include "Enemy/CBPatrolRigDebugVisualizer.h"
...
UCBEnemyPatrolRigComponent::UCBEnemyPatrolRigComponent()
{
	...
	Visualizer = CreateDefaultSubobject<UCBPatrolRigDebugVisualizer>(TEXT("PatrolVisualizer"));
#endif
...
void UCBEnemyPatrolRigComponent::BeginPlay()
{
	Super::BeginPlay();
	...
	if (auto* Owner = GetOwner())
	{
		if (auto* World = Owner->GetWorld())
		{
			...
			if (auto* Enemy = World->SpawnActorDeferred<ACBEnemyCharacterBase>(EnemyToSpawn.Get(), SpawnTransform, 
...
void UCBEnemyPatrolRigComponent::OnComponentCreated()
{	
	Super::OnComponentCreated();
	PatrolSpline->SetupAttachment(this);
	PatrolTriggerBox->SetupAttachment(this);
	AttackTriggerBox->SetupAttachment(this);
#if WITH_EDITORONLY_DATA
...
void UCBEnemyPatrolRigComponent::OnComponentDestroyed(bool bDestroyingHierarchy)
{
	...
	if (AttackTriggerBox != nullptr)
	{
		AttackTriggerBox->DestroyComponent();
		AttackTriggerBox = nullptr;
	}
#if WITH_EDITORONLY_DATA
