...
/**
 * An auto discovered settings object specifically for CB. We can store any project specific objects and properties here.
 * Those soft object pointers will be written to an .ini file used by the game
 */
UCLASS(config=Game, defaultconfig, meta=(DisplayName="CB Project Settings"))
class CB_API UCBDeveloperSettings : public UDeveloperSettings
{
	GENERATED_BODY()
	
public: 
	UFUNCTION(BlueprintCallable, Category = "CB")
	static const UCBDeveloperSettings* GetCBDeveloperSettings(); 
...
public:
	...
	UPROPERTY(Config, EditAnywhere, Category = "CB")
