 ...
 * 
 * For CB, we primarily use walking, falling, and jumping. 
 *
 ...
 */
UCLASS()
class CB_API UCBCharacterMovementComponent : public UCharacterMovementComponent
{
	GENERATED_BODY()
...
public: 
	 ...
	 * 
	 * For CB, this handles the custom jump logic to simulate a "platformer" style jump. 
	 * 
...
public: 
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, meta=(ClampMin="1", UIMin="1"), Category="CB")
	float ApexJumpHeight = 500.0f; 
	
	// The amount of time that the player has to input a jump input after walking off a ledge 
	UPROPERTY(EditAnywhere, BluePrintReadWrite, meta=(ClampMin="0", UIMin="0"), Category="CB")
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, meta=(ClampMin="0.1", UIMin="0.1"), Category="CB")
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, meta=(ClampMin="0", UIMin="0", EditConditionHides, EditCondition = "GravityEasingType == EEasingFunc::EaseIn||GravityEasingType == EEasingFunc::EaseOut||GravityEasingType == EEasingFunc::EaseInOut"), Category="CB")
	double FallGravityEaseBlend = 2.0; 

	// The step applied when using the Step easing function type
	UPROPERTY(EditAnywhere, BlueprintReadWrite, meta=(ClampMin="0", UIMin="0", EditConditionHides, EditCondition="GravityEasingType == EEasingFunc::Step"), Category = "CB")
	int FallGravityStep = 2; 

protected:

	// Ticks any movement related timers that are custom to the CB Character Movement Component 
