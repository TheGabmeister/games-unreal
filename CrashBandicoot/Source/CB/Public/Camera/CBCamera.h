...
class UCBCharacterMovementComponent;
...
class CB_API ACBCamera : public AActor
{
	GENERATED_BODY()
...
public:	
	...
	void SetFollowTarget(ACBPlayerCharacter* Target) { FollowTarget = Target; }

	// Sets the world settings from the world this camera belongs to 
	void SetWorldSettings(ACBWorldSettings* WorldSettings) { CBWorldSettings = WorldSettings; }
...
private: 
	...
	UPROPERTY(Transient)
	TObjectPtr<ACBPlayerCharacter> FollowTarget;

	// The follow character's movement component
	UPROPERTY(Transient)
	TObjectPtr<UCBCharacterMovementComponent> FollowCharacterMovementComponent; 
